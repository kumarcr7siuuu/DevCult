...
import { ArrowLeft, Check, Star, Users, Zap, Globe } from "lucide-react";
import React from "react";

export default function KisanYugPage() {
    return (
                ...
                <section className="min-h-screen flex flex-col lg:flex-row items-center justify-center px-4 lg:px-20 pt-32 lg:pt-0 gap-12">
                    ...
                    >
                        ...
                        <h1 className="text-6xl md:text-8xl font-serif text-transparent bg-clip-text bg-gradient-to-r from-white via-white/80 to-white/40">
                            DIGITAL <br />
                            <span className="italic font-light">HARVEST</span>
                        </h1>

                        <p className="text-xl text-white/60 max-w-lg leading-relaxed">
                            Empowering farmers with direct market access and fair pricing.
                            KisanYug bridges the gap between field and fork.
                        </p>

                        <div className="flex gap-4">
                            <Link
                                href="https://github.com/kumarcr7siuuu/KisanYug"
                                target="_blank"
                            ...
                            <Link
                                href="https://www.figma.com/design/rQFiNqeGMDLUws9lk7ACNU/SIH?node-id=0-1&t=eVZfl9n4xk5VDjdh-1"
                                target="_blank"
                    ...
                    >
                        <div className="relative w-[300px] md:w-[380px] aspect-[1/2]">
                            {/* Make image fit cover */}
                            <Image
                                src="/kisanyug-hero.png"
                                alt="KisanYug UI"
                                fill
                    ...
                    <div className="max-w-7xl mx-auto w-full">
                        ...
                        >
                            <h2 className="text-5xl md:text-7xl font-sans font-bold mb-6">
                                POWER TO <br />
                                <span className="text-white/30">THE FARMER</span>
                            </h2>
                            <p className="text-white/60 max-w-xl text-lg md:mx-0 mx-auto">
                                Experience inclusive technology built for rural India.
                                Bi-lingual support ensures no one is left behind.
                            </p>
                        </motion.div>
...
function InteractiveShowcase() {
    // Default to index 1 (Language Support) to highlight it
    const [activeIndex, setActiveIndex] = React.useState<number>(1);

    const features = [
        {
            title: "Join the Network",
            description: "Seamless onboarding for Farmers, Retailers, and Consumers.",
            image: "/kisanyug-1.png",
            tag: "Onboarding",
            id: 0
        },
        {
            title: "Inclusive Design",
            description: "Fully accessible with native Hindi & English support.",
            image: "/kisanyug-2.png",
            tag: "Bi-Lingual",
            id: 1,
            highlight: true // Custom flag for visual emphasis
        },
        {
            title: "Smart Marketplace",
            description: "Real-time produce tracking and order requests.",
            image: "/kisanyug-3.png",
            tag: "Market",
            id: 2
        }
    ];
    ...
    const getVariants = (index: number) => {
        ...
        // Base Configuration
        const xSpacing = 120; // Keep tight overlap
        const scaleStep = 0.15;
        const zBase = 50;

        // Calculate props based on distance from active
    ...
    const handleDragEnd = (event: any, info: any) => {
        ...
        // Swipe Left (Drag negative) -> Go Next
        if (offset < -50 || velocity < -500) {
            if (activeIndex < features.length - 1) {
                setActiveIndex(activeIndex + 1);
            }
        }
        // Swipe Right (Drag positive) -> Go Prev
    ...
    return (
        <div className="relative h-[600px] w-full flex items-center justify-end pr-8 md:pr-32 perspective-1000 overflow-hidden text-left">
            {features.map((feature, index) => (
                <motion.div
                    ...
                    // Mobile: w-[260px], Desktop: w-[340px]
                    className="absolute w-[260px] md:w-[340px] aspect-[9/16] cursor-pointer origin-center touch-pan-y"
                    style={{ right: '35%' }} // Anchor
                ...
                >
                    ...
                    {/* Visual Highlight for Bi-Lingual Support */}
                    {feature.highlight && activeIndex === index && (
                        <motion.div
                            initial={{ opacity: 0, scale: 0.8 }}
                            animate={{ opacity: 1, scale: 1 }}
                            className="absolute -top-12 left-1/2 -translate-x-1/2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/20 flex items-center gap-2 whitespace-nowrap"
                        >
                            <Globe size={16} className="text-blue-400" />
                            <span className="text-sm font-bold">Bi-Lingual Support</span>
                        </motion.div>
                    )}
                </motion.div>
            ))}
